...
while True:
    ...
    try:
        ...
        if (max_price - current_price) / max_price * 100 >= threshold:
            logging.warning('Цена фьючерса XRP/USDT упала на 1% или более от максимальной цены за последний час!')
            print('Цена фьючерса XRP/USDT упала на 1% или более от максимальной цены за последний час!')
    except:
        logging.error('Ошибка при получении данных')
    # Задержка перед следующим циклом
    time.sleep(1)
    print(f'Цикл выполнен, цена XRPUSDT составляет { current_price}')